...
fig = plt.figure(
    figsize=(13 / 2.54, 5.2 / 2.54), dpi=300
)  # initialize a figure
...
ax.plot(x_noise, pdf_noise, label="Noise")
ax.plot(x_sn, pdf_sn, label="Signal + Noise")
...
ax.set_xlim(np.amin(x_noise), np.amax(x_sn))
ax.set_ylim(bottom=np.amin(pdf_sn))
ax.set_yscale("log")
ax.set_xticks(range(-6, 12, 2))
ax.set_xticklabels([f"{x:d}" for x in range(-6, 12, 2)])

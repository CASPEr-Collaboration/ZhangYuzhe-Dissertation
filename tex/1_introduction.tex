...
To make the connection between theory and experiment explicit, physical constants such as the speed of light and Planck's constant are retained in the equations throughout this thesis.
...
\begin{equation}
    \Delta E = \mu B_0 = \hbar \gamma B_0
    \,,\label{eq:Zeeman}
\end{equation}
where  $\mu$ is the magnetic moment and $\gamma$ is the nuclear gyromagnetic ratio.
...
For protons\,\cite{NIST_CODATA2022},
\begin{equation}
    \gamma = 2\pi\times\SI{42.577478461(18)}{\MHz/\tesla}
    \,,\label{eq:gamma_proton}
\end{equation}
so a bias magnetic field of \SI{31.67}{\milli\tesla}, as used in this work, gives $\nu_L \approx \SI{1.348}{\MHz}$.

\begin{figure}
    \centering
    \includegraphics[width=.5\linewidth]{figures/Zeeman_effect.png}
    \caption{Zeeman splitting of a spin-$1/2$ nucleus in a magnetic field $B$. The energy difference $\Delta E$ between the two spin states is proportional to $B$.}
...
Hyperpolarization techniques---such as spin-exchange optical pumping (SEOP) or parahydrogen-induced polarization (PHIP)---can boost $p$ by several orders of magnitude\,\cite{Eills2023_Spin_Hyperpolarization}.
...
With lower nuclear polarization or a smaller sample volume, the SPN can become a limiting factor for the sensitivity of the experiment.

\subsection{Bloch equations}\label{sec:intro:bloch_eq}
The evolution of the nuclear spin magnetization $\mathbf{M}$ in magnetic fields is governed by the Bloch equations\,\cite{Abragam1961_NuclearMagnetism}:
\begin{align}
    \frac{dM_x}{dt} &= \gamma\left(\mathbf{M}\times\mathbf{B}_\mathrm{eff}\right)_x - \frac{M_x}{T_2}
    \,,\label{eq:bloch_x}\\
    \frac{dM_y}{dt} &= \gamma\left(\mathbf{M}\times\mathbf{B}_\mathrm{eff}\right)_y - \frac{M_y}{T_2}
    \,,\label{eq:bloch_y}\\
    \frac{dM_z}{dt} &= \gamma\left(\mathbf{M}\times\mathbf{B}_\mathrm{eff}\right)_z - \frac{M_z-M_\mathrm{eqb}}{T_1}
    \,,\label{eq:bloch_z}
\end{align}
...
In most NMR experiments, the bias magnetic field $\mathbf{B}_0$ is much larger than any other applied magnetic field.
...
In most NMR experiments, a bias magnetic field is applied, which defines the longitudinal direction, and the transverse plane is perpendicular to it\footnote{There are NMR experiments where a zero to ultra-low field (ZULF)\,\cite{Barskiy2025_ZULFNMRreview} is applied to study phenomena like the spin-spin interaction, which are difficult to observe at high fields.}.
...
Its rate is characterized by the relaxation time $T_1$ or relaxation rate $1/T_1$.
The ``return'' can happen in two directions: when the longitudinal magnetization is smaller than the equilibrium value, it grows toward equilibrium; when it is larger (hyperpolarized), it decays.
...
Its duration can be tuned by adjusting settings such as the bias magnetic field, temperature, and sample composition.
...
%     \caption{Simulated $T_1$ relaxation of a hyperpolarized sample in a static bias magnetic field. The axial magnetization $M_z$ is normalized by the equilibrium magnetization $M_\mathrm{eqb}$.}
...
Transverse relaxation, which determines how quickly coherent precession dephases, is characterized by several different time scales, including $T_2$, $T_\delta$ and $T_2^*$.
...
For brevity, relaxation processes are referred to by the symbols of their characteristic time scales in this thesis, for example, $T_1$ relaxation.
...
% For sample initially polarized by the bias magnetic field
...
In a common NMR experiment, the sample is first thermally polarized in a bias magnetic field, then perturbed by excitation pulses to generate a precession signal in the transverse plane, and finally allowed to relax back to equilibrium.
...
The resulting free-decay signal has an exponential envelope ($\exp(-t/T_2^*)$) and can be used to measure $T_2^*$ and the Larmor frequency.
The process is illustrated in Figure~\ref{fig:pulsed_NMR-schematic}.
\begin{figure}
    \centering
    \includegraphics[width=.99\textwidth]{figures/pulsed_NMR-schematic.pdf}
    \caption{Schematic of a free-decay pulsed NMR sequence in the laboratory frame. One excitation pulse tips the magnetization into the transverse plane, where it precesses and decays with time constant $T_2^*$. The signal is measured by a pickup coil.}
    \label{fig:pulsed_NMR-schematic}
\end{figure}

If the measured quantity is the voltage induced in a pickup coil by spin precession, the signal is called free-induction decay (FID).
However, in this thesis, the measured quantity is the magnetic flux.
Therefore, the more general term ``free decay'' is used.
...
The reversed dephasing, or ``refocusing,'' produces a spin echo\,\cite{Hahn1950_SpinEchoes}, as illustrated in Figure~\ref{fig:spin_echo-schematic}.
% Spin echo uses a $\pi$ pulse after a $\pi/2$ pulse to reverse dephasing due to field inhomogeneity.
This spin-echo scheme separates spin-spin relaxation from relaxation caused by field inhomogeneity.

\begin{figure}
    \centering
    \includegraphics[width=.99\textwidth]{figures/spin_echo-schematic.pdf}
    \caption{Schematic of the spin-echo sequence in the laboratory frame. Black circular arrows indicate the precession. Colored arrows represent the magnetization of small sample packets precessing at different Larmor frequencies. The $\pi$ pulse reverses dephasing from static field inhomogeneity and produces echoes. The indices in brackets indicate the time sequence of the pulses and echoes.}
    \label{fig:spin_echo-schematic}
\end{figure}

% \subsubsection{CPMG sequence}\label{sec:intro:NMR:scheme:CPMG}
The Carr--Purcell--Meiboom--Gill (CPMG)\,\cite{CarrPurcell1954_DiffusionFreePrecession, MeiboomGill1958_ModifiedSpinEcho} sequence extends the spin-echo scheme by applying a $\pi/2$ pulse followed by a train of $\pi$ refocusing pulses.
...
This interpretation is not yet unique, but it highlights the challenges facing $\Lambda$CDM at small scales.
...
Why does quantum chromodynamics (QCD) appear to conserve $CP$ ($C$ and $P$ are the charge conjugation and parity operators, respectively) so precisely even though the theory allows a $CP$-violating term that should otherwise be observable?
...
To distinguish this axion model from other axionlike particles, it is also referred to as the ``QCD axion'' or ``PQ-axion'' in the literature.
...
ALPs share the oscillatory and weakly coupled character of the QCD axion, but their mass range can be much wider, since the mechanism of ALP production differs from that of the QCD axion.
The ALP mass range has been investigated using astronomical observations\,\cite{Marsh2016_AxionCosmology} and theoretical studies\,\cite{Zhitnitsky1980_WeinbergCP}.
However, in this thesis, the ALP mass is regarded as a free parameter, and the search is performed in a range to which the experiment is sensitive.

\subsubsection{Wave nature}

A nonrelativistic ALP with mass $m_a$ and velocity $\boldsymbol{v}_a$ ($v_a \ll c$) has energy $E_a \simeq m_a c^2+\tfrac{1}{2}m_a v_a^2$ and a Compton frequency $\nu_a=E_a/h$, where $h$ is Planck's constant and $c$ is the speed of light in vacuum.
...
The minimum de~Broglie wavelength is $\lambda_{\mathrm{dB}} \sim \SI{7e-5}{\meter}$ for ALPs in the Milky Way ($m_a \leq \SI{10}{\eV/c^2} = \SI{2e-35}{\kilogram}$)\,\cite{Zhang2026_Dissertation_ipynb_1_introduction}.
For the ALP mass range relevant to this thesis ($m_a \leq \SI{2.5}{\micro\electronvolt}$), the de~Broglie wavelength is $\lambda_{\mathrm{dB}} \geq \SI{270}{\meter}$, much larger than the size of the laboratory.
...
\begin{align}
    ...
    &=\sqrt{\frac{2\hbar^3c}{m_a}}\,
      \Re\left[\Psi(\mathbf{r},t)\right]
    \,,\label{eq:ALP_single_wavefunction}
\end{align}
where $\Psi$ is the wavefunction of a single ALP.
...
For ALP masses relevant to this thesis, $m_a \leq \SI{2.5}{\micro\electronvolt}$ and velocities $v_a \leq v_\mathrm{esc} = \SI{550}{\km/\s}$, the occupation number is $N_a \geq \SI{3e27}{}$, much larger than unity\,\cite{Zhang2026_Dissertation_ipynb_1_introduction}.
...
For $N_a$ ALPs that are collectively described by a wavefunction normalized as $\int|\Psi|^2\,d^3r=1$, the associated scalar field is the appropriately scaled real part of the wavefunction, as in Equation~\eqref{eq:ALP_single_wavefunction}:
\begin{equation}
    a(\mathbf{r},t)
...
\begin{equation}
    ...
    \,\label{eq:ALP_field}
\end{equation}
over a region where the ALP density can be treated as uniform.
...
Chapter~\ref{chap:theory} develops this behavior for the Milky Way ALP halo and the Earth-bound ALP halo.
...
This thesis focuses on the gradient coupling to nucleons, also known as the ``ALP-wind'' coupling.
...
In this form of the Hamiltonian, $g_{\mathrm{aNN}} a_0$ becomes dimensionless.
...
In some papers, the nuclear spin operator $\mathbf{I}$ is written as $\hbar {\boldsymbol\sigma_n}/2$, where ${\boldsymbol\sigma_n}$ is the dimensionless Pauli spin operator.
...
\subsection{ALP searches with spin haloscopes}

Depending on the theoretical model, different experimental schemes are adopted to search for ALP-induced spin precession\,\cite{Kimball2023_search}.
...
Other examples are Noble and Alkali Spin Detectors for Ultralight Coherent darK matter (NASDUCK) using a Floquet NMR quantum detector\,\cite{bloch2105nasduck} and the QUaerere AXions (QUAX) experiment probing the possible ALP-electron spin interaction\,\cite{Crescini2020_Ferromagnetic}.

The Cosmic Axion Spin Precession Experiment (CASPEr) is a project proposed in 2014\,\cite{CASPErProposal2014} that uses nuclear spins to search for ALP-induced spin precession.
...
CASPEr-Gradient or CASPEr-G searches for the ALP-fermion gradient coupling described by Equation~\eqref{eq:HaNN}.
Two setups in Mainz, Germany, cover different ALP-mass ranges.
The CASPEr-Gradient-LowField experiment uses bias magnetic fields up to \SI{0.1}{\tesla}, corresponding to Larmor frequencies up to \SI{4.3}{\MHz}.
...
The cryostat contains a liquid helium (L\ch{He}) reservoir, excitation and pickup coils, and a SQUID magnetometer.
A superconducting magnet produces the bias magnetic field $\mathbf{B}_0$, which sets the Larmor frequency and can be varied to scan ALP masses.

% resonance and broadband scheme
The resonance scheme is the main search strategy for CASPEr-Gradient-LowField.
The bias magnetic field is stepped through a range of values, and the NMR response is recorded at each step while no excitation is applied.
A resonant signal appears as a peak in the transverse magnetization when $\nu_L$ matches $\nu_a$.

The broadband scheme, on the other hand, continuously excites the NMR resonance without changing the bias magnetic field, looking for signals that are modulated by the ALP field.
...
Beyond ALP-fermion coupling, CASPEr-Gradient, as a sensitive magnetometer experiment, has the potential to search for other exotic physics, such as the axion-photon coupling and dark photons\,\cite{BerlinKahn2025_NewTechAxionDarkPhoton}.
...
In this thesis, it is assumed that ALPs constitute all of the local dark matter, and the ALP mass and coupling strengths are treated as free parameters.
...
If no ALP \textbf{candidate} is found, the search sets an \textbf{exclusion} limit on the ALP coupling strength over the searched mass range.
The terms ``limit'' and ``constraint'' are used interchangeably with ``exclusion'' in the literature.
...
Figure~\ref{fig:AxionProton_with_Projections}, as an example of the parameter space searched by haloscopes, summarizes the current status of the ALP-proton mass-coupling parameter space.
...
\begin{figure}
    ...
    \label{fig:AxionProton_with_Projections}
\end{figure}

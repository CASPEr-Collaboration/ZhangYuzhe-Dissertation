...
In a common NMR experiment, the sample is first thermally polarized in a bias magnetic field, then perturbed by excitation pulses to generate precession signal in the transverse plane, and finally allowed to relax back to equilibrium.
...
\begin{figure}
    ...
    \caption{Schematic of free-decay pulsed NMR sequence in the laboratory frame. One excitation pulse tips the magnetization into the transverse plane, where it precesses and decays with time constant $T_2^*$. The signal is measured by a pickup coil.}
...
\begin{figure}
    ...
    \caption{Schematic of the spin-echo sequence in the laboratory frame. Black circular arrows indicate the precession. Colorful arrows stand for magnetization of small sample packets precessing at different Larmor frequencies. The $\pi$ pulse reverses dephasing from static field inhomogeneity and produce echos. The indices in brackets indicate the time sequence of the pulses and echos.}
...
A nonrelativistic ALP with mass $m_a$ and velocity $\boldsymbol{v}_a$ ($v_a \ll c$) has energy $E_a \simeq m_a c^2+\tfrac{1}{2}m_a v_a^2$, and Compton frequency $\nu_a=E_a/h$, where $h$ is Planck's constant and $c$ is the speed of light in vacuum.

...
\begin{jointwork}
    Some material in this chapter is adapted from publications by the author and collaborators. 
    The apparatus description is adapted from Ref.\,\cite{Walter2025_CASPErLFthermal}.
...
Surrounding the solenoid, 8 superconducting shim coils (named Z1, Z2, X, Y, XY, XZ, YZ, and X$^2$-Y$^2$) are used to reduce the field inhomogeneity to less than \SI{2}{\ppm} over a spherical volume with a radius of \SI{4}{\mm}.
...
The larger-than-design inhomogeneity results from insufficient shimming duration, use of a longer sample (\SI{24}{\mm} in height), and the presence of superconducting coils.

Helmholtz coils wound outside the cryostat provide a controllable offset on $B_0$, enabling fine-tuning of the Larmor frequency without ramping the main solenoid, which is time-consuming and can change the magnetic field by an unknown amount.
...
The SQUIDs and the fiberglass coil stacks on which Helmholtz coils, excitation coils, and the pickup coil are mounted are attached to the outer surface of the VTI.
...
\begin{figure}[htb]
    ...
    \caption{(a) Cryostat for maintaining superconducting operating temperatures.
    The L\ch{He} bath provides a \SI{4.2}{\kelvin} environment for the superconducting apparatus.
    (b) Superconducting apparatus including the main coil, shim and excitation coils, SQUIDs with a \ch{Nb} capsule, \ch{Nb} connecting wires, and a pickup coil in first-order gradient configuration made of \ch{Nb} wires.}
...
The radius of each coil is \SI{14}{\milli\meter}.
The opening angle of each saddle-pair coil is \SI{120}{\degree}.
...
\begin{figure}[htb]
    ...
    \caption{(a) Simplified schematic of the gradiometer with the sample positioned at its center.
    (b) Details of the gradiometer loop connections showing the three-coil saddle-pair configuration with opposing windings for noise rejection.}
...
The SQUID is operated in flux-locked-loop (FLL) mode during measurements, i.e., a feedback voltage is applied to the SQUID to maintain the magnetic flux in the coils so that it remains at the linear operating point of its $V-\Phi$ characteristic, as illustrated in Figure~\ref{fig:squid_schematic}.
The feedback voltage is the measured output.

\begin{figure}[htb]
    \centering
    \includegraphics[width=.8\textwidth]{works/master-thesis/figures/02_Apparatus/SQUID_Schematic_VPhi_FLL.png}
    \caption{(a) $V-\Phi$ characteristic of a dc-SQUID showing the operating point W where the feedback voltage has an approximately linear dependence on magnetic flux.
    (b) Directly coupled FLL circuit with built-in pre-amplifier and integrator for maintaining the SQUID at the linear operating point.
    Reproduced from \cite{squidhandbookintro} with permission.
    }
...
\end{equation}
where $R_\mathrm{f}$ is the feedback resistance, $M_\mathrm{f}$ is the feedback-coil mutual inductance, $\Phi_\mathrm{in}$ is the input magnetic flux in the SQUID, $M_\mathrm{in}$ is the mutual inductance between the pickup and SQUID input coils, and $L_\mathrm{pick}$, $L_\mathrm{in}$ are the respective coil inductances.
The relevant parameters are summarized in Table~\ref{tab:SQUID_params}.
...
The outputs of the two channels are recorded at a sampling frequency of \SI{13.4}{\kHz} and are used to generate spectra via fast Fourier transformation (FFT).
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item Remote switching of the shim power supply on and off
        \item Connection and disconnection of communication between the power supply and the shim coils
    ...
    \begin{enumerate}
        ...
        \item Recording rf pulse timing using an oscilloscope and storing it in HDF5 format
...
The control system orchestrates multiple measurement schemes, including pulsed-NMR diagnostics for sample characterization, automated shimming procedures with feedback optimization, ALP measurements, and magnetic field monitoring.
Additionally, the system includes capabilities for NMR kinetic simulations, stochastic ALP field simulations, computation of apparatus properties, and synchronized file management across multiple terminals.
...
In addition to the ALP coherence time $\tau_a$, the signal is also affected by the sample relaxation time $T_2^*$.
...
Taking Equations~\eqref{eq:SQUID_beta} and \eqref{eq:xi_final} into account, the following relationship is obtained:
\begin{equation}
    V_a = \beta\,\mu_0 M_0 \Omega_a T_2^* \left(\frac{\tau_a}{\tau_a + T_2^*}\right)^{1/2} \propto g_{\mathrm{aNN}} T_2^* \left(\frac{\tau_a}{\tau_a + T_2^*}\right)^{1/2}
    \,,\label{eq:Va_gaNN_Tcoh}
\end{equation}
where $V_a$ is the SQUID feedback voltage corresponding to the ALP-induced transverse magnetization.
In the following discussion, the experimental parameters $\beta$ and $M_0$ are treated as constants and therefore omitted for simplicity, and the ALP-induced signal is expressed as $g_{\mathrm{aNN}} T_\mathrm{coh}$ where
\begin{equation}
    T_\mathrm{coh} = T_2^*  \left(\frac{\tau_a}{\tau_a + T_2^*}\right)^{1/2}
    \,.\label{eq:T_coh}
\end{equation}
is the effective coherence time of the ALP-induced signal.
...
The noise is characterized by its standard deviation,
\begin{equation}
    \sigma = \sqrt{\frac{1}{T \Delta\nu}}\,\sigma_{0}
    \,,\label{eq:sigma}
\end{equation}
where $\sigma$ and $\sigma_{0}$ are the noise standard deviations with and without averaging, respectively.
Since $N_{\Delta\nu} \gg 1$, the noise distribution approaches a Gaussian distribution according to the central limit theorem.
The coupling strength $g_{\mathrm{aNN}}$ corresponding to a \SI{5}{\sigma} signal can be found using the signal-to-noise ratio (SNR) condition
\begin{equation}
    \frac{\mathrm{PSD}}{\sigma} = 5
    \,, \label{eq:SNR_condition}
\end{equation}
which can be expanded using Equations~\eqref{eq:S_PSD}, \eqref{eq:Gamma_signal}, and \eqref{eq:sigma} to
\begin{equation}
...
\end{equation}
The implications of Equation~\eqref{eq:g_aNN_sensitivity_2regimes} can be summarized as follows:
\begin{enumerate}
    \item When the measurement duration $T$ is much longer than the sample relaxation time $T_2^*$ and the ALP coherence time $\tau_a$, the sensitivity is dominated by the term $T^{-1/4}$, indicating a slower improvement with increasing measurement duration.
    \item The sensitivity improves with a longer sample relaxation time $T_2^*$. The improvement is more significant when $T_2^* \ll \tau_a$.
\end{enumerate}
...
\begin{enumerate}[label=\Alph*.]
    \item Pulsed-NMR measurement to determine the resonance frequency and the relaxation time $T_2^*$.
...
It can be estimated from the NMR linewidth, the ALP linewidth, and the noise profile.
For non-amplifiable noise (white noise with a flat spectrum), the experiment is sensitive over a bandwidth approximately equal to the maximum of the NMR linewidth and the ALP linewidth.
For noise amplified by NMR, the experiment is equally sensitive over a bandwidth much larger than the NMR linewidth.
Choosing a step size larger than the sensitive bandwidth leaves gaps in sensitivity; choosing a smaller step size multiplies the number of steps and the dead time between steps without improving sensitivity.
Therefore, the scan-step size is set to be approximately equal to the sensitive bandwidth of a single step.
Examples of amplifiable and non-amplifiable noise are illustrated in Figure~\ref{fig:scan-flat_peak_sensi}.
...
\begin{figure}[htb]
    ...
    Rows (top to bottom): Frequency spectra of the square of magnetic signal amplification $\eta(\nu)$ in panels (a) and (d), PSD noise in panels (b) and (e), and the corresponding ALP coupling strength $g_\mathrm{a}$ sensitivity in panels (c) and (f). Offsets have been added to the noise spectra for clarity.
...
How can the scanning strategy be optimized to maximize the figure of merit?
...
Given a fixed total measurement time, the relaxation time $T_2^*$ can be tuned to maximize the parameter space coverage.
...
A short relaxation time $T_2^*$ may allow for a larger scan-step size and fewer steps in total, and therefore increases the measurement time per step.
...
With a fixed total measurement time $T_{\mathrm{tot}}$, the measurement duration can be chosen to be proportional to the ALP coherence time: $T \propto \tau_a \propto \nu^{-1}$.\footnote{Measurement time can also be allocated uniformly across frequency steps, which makes no significant difference in the final sensitivity. A quantitative comparison of the allocation schemes is given in Ref.\,\cite{Yuzhe2023_FrequencyScanning}.}
...
\begin{equation}
    ...
    \,,\label{eq:scan_rate}
\end{equation}
where $\kappa$ is a constant to be determined later from the total measurement time and the scan range.
...
\begin{equation}
    \kappa = \frac{T_\mathrm{tot}}{\pi Q_a T_2^* } \left[\ln\left(\frac{\nu_\mathrm{end}}{\nu_\mathrm{start}}\right)\right]^{-1} \propto T_\mathrm{tot} \left(T_2^*\right)^{-1}
    \,.\label{eq:kappa_regime2}
\end{equation}
Using Equations~\eqref{eq:g_aNN_sensitivity_2regimes} and \eqref{eq:kappa_regime2} and omitting constant factors, the sensitivity in this regime is
\begin{equation}
    \left|g_{\mathrm{aNN}}\right| \propto
    T_{\mathrm{tot}}^{-1/4}\, \left(T_2^*\right)^{-1/2}\, \tau_a^{-1/2}
    \,. \label{eq:g_aNN_sensitivity_scan_regime2}
\end{equation}
From Equation~\eqref{eq:g_aNN_sensitivity_scan_regime2}, the sensitivity still improves with longer $T_2^*$, which is consistent with the estimate in the preceding discussion.
...
%     The sensitivity scales as $T_{\mathrm{tot}}^{-1/4}$ in all regimes, a consequence of the long-measurement-duration limit $T \gg \tau_a,\,T_2^*,\,T_2$\,\cite{CASPErProposal2014,Aybas_2021_Quantum_sensitivity}.

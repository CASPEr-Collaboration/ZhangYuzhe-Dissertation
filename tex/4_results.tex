...
\begin{enumerate}
    \item \textbf{Window on the time-series.} If values in the data are found to be \texttt{NaN}, infinite values, jumps exceeding a certain threshold, or drifts, a window is applied on the time-series to set the values to zeros, so their effects are excluded. 
...
From the NMR characterization, the SQUID magnetic flux when the sample magnetization is tipped to the transverse plane, $\Phi_\perp(\nu)$, can be obtained by the PSD of the free decay signal.
For a given ALP coupling strength $g_{\mathrm{aNN}}$, the tipping angle $\xi$ of the sample magnetization is determined by the Rabi frequency of the ALP field $\Omega_a \propto g_{\mathrm{aNN}}$ (given in Equations~\eqref{eq:Omega_a_MW} for Milky Way ALP and \eqref{eq:Omega_a_EH} for Earth-halo ALP) and effective coherence time $T_\mathrm{coh}$ (given in Equation~\eqref{eq:T_coh}). 
...
In the absence of a $n\,\sigma$ ALP signal at a certain confidence level, 
\begin{equation}
    \left(\Omega_a T_\mathrm{coh} \, \Phi_\perp(\nu)\right)^{1/2} \leq n\,\sigma
    \,,\label{eq:gaNN_lim_equation}
\end{equation}
solving which gives the constraint on the ALP coupling strength $g_\mathrm{aNN}$ over frequency. 
% \begin{equation}
...
For Milky Way ALP field, the expected linewidth near \SI{1.3}{\MHz} is approximately \SI{1}{\Hz}, corresponding to a coherence time of approximately \SI{0.3}{\s}.
...
\begin{figure}[htb]
    ...
    Blue dot with error bar: expected ALP signal powers in the PSD, according to Equations~\eqref{eq:squidpickup} and \eqref{eq:SQUID_beta}, for the reference case of an ALP gradient coupling to proton of $g_\mathrm{ap}=\SI{1e-9}{\GeV^{-1}}$.
    Uncertainties for these values were propagated from errors of the spin relaxation times $T_2$ and $T_2^*$, sample Larmor frequency and NMR signal power. Reproduced from Ref.\,\cite{Walter2025_CASPErLFthermal} with permission. }
...
\begin{figure}[htb]
    ...
    \caption{Example of data processing with fake ALP signal injected at the center of the spectrum. Top to bottom: lineshape of normalized ALP field, free decay PSD, PSD of the measurement, and NPE. }
...
In order to include information of RFI and other non-white-noise structures, the data analysis searched for ALP signals in the frequency range of \SI{1.348570}{\MHz}\pm\SI{350}{\Hz}, much larger than the combined NMR linewidth of $\pm\SI{120}{\Hz}$. 
This frequency range corresponds to \SI{700}{} Milky Way ALP linewidths at this frequency. 
In the absence of ALP signal, the number of outliers is expected to be \SI{.7}{} out of \SI{700}{} based on the false alarm rate (\SI{.1}{\percent}) obtained in the MC simulations. 
In real analysis, \SI{58}{} outliers were found in the searched frequency range, which is much larger than the expected number of outliers due to non-white-noise structures that are not removed by the SG filters. 
After validation check, \SI{6}{} candidates were found out of the outliers, listed in Appendix~\ref{app:candidates}. 
...
\begin{figure}[htb]
    ...
    \caption{ALP gradient-proton coupling excluded (indicated by the shaded region) at \SI{95}{\%} C.L. by the measurement at December 23, 2022. The sensitivity is good at \SI{1.348690}{\MHz} because the measurement duration was longer at this frequency. The sensitivity is worse at the edges of the scanned frequency range because the NMR response is weaker there. }
    \label{fig:ALPlimits}
\end{figure}
...
As mentioned in Section~\ref{sec:theory:earthALP:coherence}, empirical results are presented in this section under the assumption that the Earth-bound ALP are mostly in the lowest-energy eigenstates, where the ALP density is significant at Earth radius.   
For the seven lowest-energy eigenstates of the Earth-bound ALP halo, the expected daily modulation\footnote{As discussed in Section~\ref{sec:theory:earthALP:periodic}, if no mechaism lifts the degeneracy in a, daily modulation would not be observed by measurements. } of the ALP Rabi frequencies is shown in Figure~\ref{fig:EH_ALP-1s_to_5s-daily_modulation-Omega_a}.
...
A typical value of Rabi frequency is approximately
\begin{equation}
    \Omega_{a,\,\perp}^{\oplus}
    \approx
    2 \pi \times \SI{.1}{\mHz}
...
The differences between the eigen-frequencies of the Earth-bound ALP halo are on the order of \SI{1}{\milli\hertz}, while the difference between $3s$ to $5s$ states is \SI{.25}{\milli\hertz}, as shown in Figure~\ref{fig:EH_ALP-state_amplitude-and-radial_wavefunctions-7states}.
...
This thesis does not further investigate these transient new physics. 
...
The time-series signal from two channels of LIA are independent and Gaussian, so the NPE follows a $\chi^2_2$ distribution. 
...
If the outlier threshold is set at $\SI{7.809}{\sigma}$, the probability of capturing a $\SI{15.06}{\sigma}$ ALP signal is \SI{95}{\percent}, while the probability of false positive (false alarm) is \SI{4e-4}{}. 
...
Those candidates that appeared in the NMR resonant windows are likely to be aliasings of RFI, since similar structures are observed in the spectra at regular intervals.
The candidates are listed in Appendix~\ref{app:candidates} for reference. 

In the absence of detection, constraints on the ALP-proton coupling can be derived from the measured NPE noise spectra and the expected signal power for the Earth-bound ALP halo model based on Equation~\eqref{eq:gaNN_lim_equation}. 

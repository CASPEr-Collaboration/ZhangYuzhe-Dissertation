...
\begin{jointwork}
    Data analysis procedures and searches for Milky Way ALP are adapted from Ref.\,\cite{Walter2025_CASPErLFthermal}.
    The development of the analysis framework took the data analysis pipelines of HAYSTAC\,\cite{Brubaker2017_HAYSTAC}, SHAFT\,\cite{Gramolin2021_Ferromagnetic}, and CASPEr-Electric\,\cite{aybas2021_SolidStateNMR} as reference. 
\end{jointwork}
...
The results including analysis of the ALP candidates and the exclusion limits are presented by the end of sections. 
\YZ{this sentence should go to main text}
...
The data analysis of CASPEr-Gradient aims at identifying potential ALP signals in the frequency spectra obtained from the measurements and setting exclusion / sensitivity on the ALP coupling in the absence of a detection.
...
\textbf{Outliers} that survive the consistency checks are considered as ALP \textbf{candidates}. 
...
\begin{itemize}
    \item \textbf{Frequency spectra generation and filtering. }Recorded time-series data are evaluated by its sanity. Sane data are used to generate frequency spectra by Fourier transform. Digital filtering, such as Savitzky--Golay (SG) filter and optimal filters are applied to the spectra to remove the baseline and optimize SNR. The filtered spectra are later used for finding outliers. 
    \item \textbf{MC simulation for determining outlier threshold.} To determine the threshold in picking up outliers in the frequency domain, MC simulation are ran. Artificial ALP signals are generated in simulations in determining the threshold. 
...
\begin{figure}
    ...
    \caption{Data-analysis pipeline for the CASPEr-Gradient search. Details of this pipeline are discussed in Section~\ref{sec:results:data-analysis:procedure}.}
...
\begin{itemize}
    \item if the ALP signal is weak compared to the noise, the data is still approximately Gaussian; 
    \item if the ALP signal is strong, the data are no longer Gaussian, and stochastic drift with a characteristic timescale of ALP coherence time is expected, as illustrated in Figure~\ref{fig:MW_axion_time_domain_stochastic}. 
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Long-term instability (``drift'').} Slow (compared with the ALP coherence time) changes of the mean or variance due to instable SQUID flux-lock-loop mode are tested within individual scan steps. 
    ...
    \item \textbf{Non-Gaussianity.} Drift-clear data are tested for consistency with the Gaussian-noise assumption used in the threshold estimation. 
\end{enumerate}
...
Note that features that do not match ALP models investigated in this thesis, such as fluctuations in period much shorter than ALP coherence time, may not be noise or faulty hardware/software behavior, but signals of other new physics. 
...
The exact formula of $\mathrm{ENBW}$ can be found at Equation (22) of Ref.\,\cite{Heinzel2002_SpectrumSpectralDensity}. 
...
This exponential moving average is also described as digital discrete-time resistor-capacitor (DTRC) filter as it resembles the low-pass response of analog resistor-capacitor (RC) filters. 
...
If the time-series is Gaussian noise, the frequency spectra should be white noise after the low-pass filter correction, and the distribution of the PSD should follow a $\chi^2$ distribution. Since there are two channels in the lock-in amplifier, the degrees of freedom of the distribution should be two.

Since the frequency-dependent attenuation in \eqref{eq:H_nu_attenuation} is only an approximation, the baseline of the PSD is not absolutely flat after the DTRC filter correction. 
The curve of the baseline is estimated by a SG filter and removed from the PSD.
In practice, the spectrum is divided into chunks of a bandwidth much larger than the expected ALP linewidth, and the SG filter of the first order is applied to each chunk to estimate the baseline.\footnote{The first order is chosen so as not to affect the ALP signal. Higher order SG filters can substract part of the ALP signal from the spectrum.} 
...
The optimal filter is a weighted average of neighboring frequency bins, with weights determined by the expected signal lineshape; this choice maximizes the SNR among linear filters for random noise with known covariance, as shown in Appendix~\ref{sec:app:optimal-filter}.
...
One of the procedures that was adopted in the former CASPEr-Gradient analysis\,\cite{Walter2025_CASPErLFthermal} was the removal of narrow spikes in the frequency spectra.
The philosophy behind this procedure is that spikes with linewidths much narrower than the expected ALP linewidth could be radio-frequency interference (RFI), and are unlikely to be ALP signals, thus can be removed to avoid false positives and reduce noise. 
However, this procedure is not applied in the analysis of the data presented in this thesis for three reasons. First and most important of all, since the stochastic nature of ALP determines the spiky shape of ALP signals as plotted in Figure~\ref{fig:MW_axion_time_domain_stochastic}. 
Moreover, RFI lines are expected to be stable in frequency and amplitude, and can be identified by repeated scans. 
Lastly, the spikes are suppressed by the optimal filtering, and did not affect the outlier search in the real analysis.
...
Such signals are considered as traces of new physics. 
\begin{figure}
...
For a virialized Milky Way ALP field, the expected linewidth near \SI{1.3}{\MHz} is approximately \SI{1}{\Hz}, corresponding to a coherence time of approximately \SI{0.3}{\s}.
...
\begin{figure}[htb]
    ...
    Uncertainties for these values were propagated from errors of the spin relaxation times $T_2$ and $T_2^*$, sample Larmor frequency $\nu_L$ and NMR signal power $P_\mathrm{NMR}$. Reproduced from Ref.\,\cite{Walter2025_CASPErLFthermal} with permission. \YZ{use sin alpha without brackets. adjust ylabel paddings }}
...
% TODO remove the two cases in EH_ALP

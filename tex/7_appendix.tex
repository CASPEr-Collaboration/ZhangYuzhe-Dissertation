...
% \begin{figure}[htb]
%     \centering
%     \fbox{\includegraphics[width=0.95\linewidth]{figures/AxionBloch_diagram.drawio.pdf}}
...
%     Reproduced from Ref.\,\cite{Zhang2026_axionbloch}, originally prepared by the author.}
...
%     \caption{Simulated $T_1$ relaxation of a hyperpolarized sample in a static bias magnetic field.
...
% In the ALP data analysis described in Section~\ref{sec:results:data_analysis}, the normalized power excess (NPE) plays the role of the test statistic.
% The null hypothesis is that a $5\,\sigma$ ALP signal is present; the alternative is that no signal is present.
% MC simulations of the NPE distribution under the null hypothesis determine the threshold $\Theta_\mathrm{RE} = 3.43$ corresponding to the \SI{95}{\%} confidence level (see Section~\ref{sec:results:data_analysis}).
...
% \chapter{Optimal filter as a weighted average}\label{sec:app:optimal-filter}

% The optimal filter used in the ALP search can be understood as the linear combination of frequency bins that maximizes the SNR.
...
% The noise in a power spectrum follows a $\chi^2_2$ distribution before averaging.
...
% For a general linear statistic $T=\sum_i w_i x_i$, the SNR is
% \begin{equation}
%     \mathrm{SNR}(T)
%     =
...
\chapter{ALP candidate lists}\label{app:candidates}
...
% TODO add coupling strength column to the table

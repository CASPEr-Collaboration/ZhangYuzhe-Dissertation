...
\input{core/document_settings} % Contains commands that are used for certain information that is printed.
...
%    \boldNumberSetsfalse % Use this value if you want variables for number sets (like N or R) to appear in blackboard bold rather than bold.
    ...
    \myTitle{Search for an Axionlike Dark Matter Candidate Using Nuclear Magnetic Resonance}{}
    ...
    \dateOfHandingIn{14.09.2026}

     % The name and affiliation of the author’s supervisor.
    \nameOfMySupervisor{Prof.\,Dr.\,Dmitry Budker}

    % A list with the names of the additional examiners.
    \additionalExaminers{Prof.\,Dr.\,Kurt Aulenbacher}

    % A short summary of the thesis. These information will be used for the PDF meta data.
    \mySubject{Search for an Axionlike Dark Matter Candidate Using Nuclear Magnetic Resonance}
    ...
    \myKeywords{ axionlike particles | dark matter | nuclear magnetic resonance }
...
\input{core/format}                % Contains commands that define the general format and layout of the thesis.
\input{core/bibliography_format}   % Contains commands for the layout of the bibliography.
\input{core/standard_packages}     % Contains the packages that this template provides.
\input{core/general_commands}      % Contains newly defined commands useful for mathematics.
\input{core/user-defined_commands} % This is where user-defined commands should go.
...
\begin{document}
    ...
    \frontmatter
    \include{core/title_page}

    \pagestyle{plain}
    
    % \addchap{Declaration of Authorship}
    \include{core/declaration_of_authorship}

    \include{0_blank}
    
    \include{0_contributions}
    \include{0_abstract}
    ...
    \include{2_theory}
    \include{2_theory-MW-ALP}
    \include{2_theory-Earth-ALP}

    \include{3_experiment}
    ...
    \include{CV/thesis_cv}
    \include{0_publications}

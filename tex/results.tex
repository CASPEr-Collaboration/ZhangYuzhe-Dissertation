...
\subsection{Overview}\label{sec:results:EarthALP20221214:overview}
...
Since the Rabi frequency $\Omega_a \approx 2 \pi \times \SI{2}{\mHz}$, the tipping angle of the magnetization can be estimated as $\theta \approx \Omega_a T_2 \approx 0.004\, \pi$.

For the \SI{1}{\hour} records used here, $\mathrm{RBW} = 1/\SI{1}{\hour} = \SI{3e-4}{\Hz}$, which is larger than the expected Earth-bound ALP linewidth. 
Therefore, the signal is unresolved in the frequency spectra and the optimal filter should not be applied.
...
\subsection{Results}\label{sec:results:EarthALP20221214:results}

Since the state population of the Earth-bound ALP halo is unknown, a few scenarios of population distribution are considered in the analysis. 

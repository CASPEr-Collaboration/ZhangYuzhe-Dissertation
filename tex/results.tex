...
\begin{jointwork}
    Data analysis procedures and searches for Milky Way ALP are adapted from Ref.\,\cite{Walter2024_LFthermal}, while the Earth-bound ALP search is adapted from Ref.\,\cite{}. \YZ{to be cited here}
\end{jointwork}
...
\subsection{Frequency spectra generation and filtering}
...
\begin{itemize}
    ...
    \item If the ALP signal is strong, the data is no longer Gaussian, and stochastic drift with a characteristic period of ALP coherence time is expected. % TODO do not use itemize?? 
\end{itemize}
...
After the baseline removal, optimal filtering is applied when the expected ALP signal is resolved by the FFT bins; otherwise the analysis uses the normalized excess power in the corresponding independent resolution element.
...
\subsubsection{Find outliers by threshold }

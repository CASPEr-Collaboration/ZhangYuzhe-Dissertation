...
The analysis follows the procedure developed for the CASPEr-Gradient-LowField search, which is similar to the pipelines used by HAYSTAC, SHAFT, and CASPEr-Electric\,\cite{Walter2024_LFthermal, Brubaker2017_HAYSTAC,Gramolin2021_Ferromagnetic,aybas2021_SolidStateNMR}. For each scan step, the procedure is:
\begin{enumerate}[label=\arabic*.]
    \item \textbf{Calibrate the measurement.} Fit the pulsed-NMR spectrum to determine the Larmor frequency, NMR linewidth, and signal power. Together with the measured $T_2$, these quantities characterize the resonant response and convert SQUID output power into an equivalent transverse magnetization.

    \item \textbf{Construct the search spectrum.} Convert the complex lock-in time series $I+iQ$ into a PSD using a discrete Fourier transform. Retain the \SI{8}{\kHz} analysis window centered on the Larmor frequency and exclude the filter artifacts near the edges. A record of duration $T_{\mathrm{meas}}$ has an FFT-bin width
    \begin{equation}
        \delta\nu_{\mathrm{FFT}}=\frac{1}{T_{\mathrm{meas}}}
        \,.\label{eq:fft_bin_width}
    \end{equation}

    \item \textbf{Suppress narrow nonwhite-noise spikes.} Compare the raw PSD with the response of a narrow Savitzky--Golay (SG) filter. Fit the residual distribution with a $\chi^2$ distribution with two degrees of freedom, identify bins inconsistent with white noise, and replace only those bins and their nearest neighbors with the local filter average \cite{SavitzkyGolay}.

    \item \textbf{Remove the broad baseline.} Estimate the slowly varying noise floor with a broad SG filter, subtract it from the spike-corrected PSD, and divide by the baseline. Signal-injection simulations are used to quantify any loss of signal power introduced by this operation.

    \item \textbf{Choose the signal statistic according to spectral resolution.} Compare the predicted signal linewidth $\Delta\nu_{\mathrm{sig}}$ with $\delta\nu_{\mathrm{FFT}}$. If the signal is resolved over several FFT bins, convolve the normalized PSD with the predicted ALP lineshape; this matched, or optimal, filter combines the signal power into a normalized power excess (NPE). If $\Delta\nu_{\mathrm{sig}}\lesssim\delta\nu_{\mathrm{FFT}}$, the lineshape is not resolved and optimal filtering is not applied; the normalized excess power in the corresponding independent resolution element is used instead. The Monte Carlo calibration must reproduce whichever branch is used.

    \item \textbf{Apply the detection threshold.} Determine the rescan threshold from Monte Carlo signal and noise realizations processed by the same pipeline. Bins or resolution elements above the threshold are classified as outliers; those within $\pm5$ NMR linewidths of resonance are ALP candidates.

    \item \textbf{Test candidates and set a limit.} Merge candidates separated by less than one signal linewidth, compare them across repeated scans, and test whether they follow the expected resonant and modulation behavior. If no candidate survives, convert the threshold power into a coupling limit after correcting for the simulated signal-recovery efficiency.
\end{enumerate}

The resolution check in step~5 is essential. For the \SI{100}{\s} standard-halo search, $\delta\nu_{\mathrm{FFT}}=\SI{0.01}{\Hz}$, whereas the expected ALP linewidth near \SI{1.35}{\MHz} is of order \SI{1}{\Hz}; the line therefore spans many FFT bins and the matched filter used in Ref.\,\cite{Walter2024_LFthermal} is appropriate. This conclusion must be reassessed for a narrower signal or a shorter record.
...
This section gives the implementation details for the resolved standard-halo signal considered in the methanol search. Each \SI{100}{\s} time series is converted to a PSD and processed according to the procedure summarized in Section~\ref{sec:results:analysis-procedure}.
...
For the \SI{1}{\hour} records used here, the FFT-bin width is $\delta\nu_{\mathrm{FFT}}\approx\SI{2.8e-4}{\Hz}$. Before applying an optimal filter to these data, the predicted Earth-bound-halo linewidth must be compared with this resolution. If the signal occupies only one FFT resolution element, the single-element excess-power statistic described in Section~\ref{sec:results:analysis-procedure} should be used instead of a lineshape convolution.
